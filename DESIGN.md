...
**An abandoned sign-in must not lock everyone else out.** Ownership used to hold until the 10-minute session deadline. A sign-in someone opened and then left blocked every other person in the company for those 10 minutes, and on Paperclip 2026.9 the page reported that as "[object Object]". The cause: `usePluginAction` throws a plain `{ code, message }` bridge error, not an `Error`. The owner's UI polls every ~1.2 s, so a sign-in with no owner activity for `ABANDONED_AFTER_MS` (30 s) now counts as abandoned. Anyone may replace it, and a non-owner's poll treats it as absent. This does not weaken the rule above: the abandoned session is cancelled with its token never minted, and the new owner gets a fresh flow.
